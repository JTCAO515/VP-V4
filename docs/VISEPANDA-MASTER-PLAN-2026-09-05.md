...
> 当前升级：[ADR-0027](adr/ADR-0027-personal-journey-assistant.md)与[新开发计划](product/assistant-upgrade-2026-09-27/README.md)替代旧导航、助手交互与新产品Pass-only约束。未冲突领域与发布合同继续适用；旧价格和Pass账本保留为历史兼容依据。

2026-09-12执行修订：[开发接入规则](agents/development-integration-policy.md)生效。当前开发无需第三方、法务或产品许可；可用账号/API即可接入验证，可逆参数由agent决定。下文历史审批要求不再阻塞开发，实际运行与发布证据仍分别记录。
...
2026-09-10 品牌增量依据：[Q1–Q38 决定输入](https://github.com/JTCAO515/VP-V4/blob/8ae95a7/docs/brand/VISEPANDA-BRAND-DISTILLATION-Q1-Q38-2026-09-10.md)、[工程调整研究](research/VISEPANDA-BRAND-ENGINEERING-ADJUSTMENTS-2026-09-10.md)。首要客群为第一次来中国的普通自由行游客；VP以旅行助手/本地帮手的身份，在行前规划与在途变化中给出适合个人的下一步。Q36明确Free共享基础显式跨Trip偏好；Q37明确一项服务任务包含必要澄清与系统修复。本文下述旧Ask数值保留为历史占位，不得直接换名为服务任务额度。Q38激活/eSIM保持未决，既有架构、确认、许可和发布门保留。
...
| 导航 | VP / Journeys / Library / Memory，默认 VP；全局 Search 承接探索，Today 在 Journeys 内；见 ADR-0027 |
...
| 商业 | Free + 用户付费订阅方向，月订阅为首轮候选；周期/价格/容量未冻结；旧Pass保持兼容，见ADR-0027 |
...
当前采用VP/Journeys/Library/Memory四Tab。VP承接持续交流与任务成果，Journeys承接目标和已确认计划，Library区分工具与资料/成果，Memory一级可见并有真实纠正作用。搜索/探索为全局入口，账号/通知/权益/隐私仍可达。AI选择经校验的内容与动作组件，不生成任意导航或按钮权限。
...
2026-09-05基线盘点的56个server TS文件中许多是fixture或内存合同；后续已有协议、身份与持久预算等增量，不能按这个旧计数判断当前实现。当前需按各模块的真实数据库、worker、provider与客户端证据推进。详见 [接口合同](program/2026-09-05/INTERFACES.md)。
...
基础明确长期偏好对Free与Pass共同可用，必须在相关问答、规划与恢复中正确使用并可纠正/撤回；这不授权自动敏感画像。Profile与Memory每字段保持唯一权威，使用任务专用投影，不能将管理列表直接注入模型。明确的“记住”指令在范围与许可清楚时不重复询问；推断和歧义先作为候选。详见[基础跨Trip偏好契约](contracts/basic-preferences-cross-trip.md)。
...
响应内容与表达遵守[VP响应规范](contracts/vp-response-policy.md)：英文为原生表达，中文保持事实、条件、否定和动作状态一致。先给核心答案/建议，再给必要限定与下一步；只在缺失信息会改变结果时追问。语气温暖、直接、沉着，幽默可为零。正文不能控制按钮权限或把未知、排队、提案、取消误写成已确认结果；生产者与客户端的旧新outcome须显式版本化。
...
**历史Pass基线与兼容参考（新包装由ADR-0027及#225承接）。**Journey Pass 为30天单次购买、不自动续费，目标参考价 **US$19.99**；US$14.99用于同类用户的单变量价格实验，不同时上两张难解释的商品卡。各地区最终显示StoreKit商品价格。
...
| 历史Ask容量占位，非新服务任务额度 | 6Ask/4小时且30/滚动7天；新单位需重测 | 300Ask/购买周期且60/滚动24小时；新单位需重测 |
| 材料解析 | 首批3份/Active Trip，每份页数/大小明示 | 首批20份/购买周期，每份页数/大小明示 |
| Guide/翻译 | 可体验，生成按Ask/媒体明示额度 | 纳入统一额度；重播缓存不扣Ask |
| 当前Trip记忆/安全约束/纠错及明确基础跨Trip偏好 | 共同可用，遵守来源/范围/许可与撤回 | 同样可用；复杂历史综合增强另有证据再定 |
| 离线关键计划/地址 | 可读 | 可读；允许的额外内容包按权利提供 |
| 导出删除、账户安全、手动编辑 | 不付费 | 不付费 |
| 人工 | 申请与队列，试点容量内由团队接受 | 不承诺自动优先、24小时或无限任务 |

上表数值来自旧Ask口径，不是已有付款用户合同，也不是已批准的新ServiceTask容量。Q37的新方向是一个明确目标及其必要澄清、系统修复构成同一服务任务；ServiceTask关联多个Turn，provider/tool attempt成本独立累积。先以记录模式验证归属与成本，数字、partial、完成后改稿、等待TTL和跨期规则明确后才启用新消费。不得双重扣次或在任务中偷偷增加消费。详见[服务任务计量契约](contracts/service-task-metering.md)。
...
下述300次/60次仍指历史Ask口径。交易身份、顺序、恢复/退款和现行购买生效机制保留；新服务容量需明确计量策略版本和数值后才写入真实商品，不把历史300Ask换名为300个完整服务任务。Q38的购买后激活与分阶段权益尚未决定，本轮不更改起算时刻。
...
2026-09-10 [HF复用计划](harness/hf-reuse/README.md)将公开评测方法和材料解析候选纳入当前队列：VPJ-72先完成中英判分与盲评工具准备，VPJ-73形成Docling采用/否决证据。保留现有TS Harness，内容质量与硬安全分开；公开数据/方法不直接变生产知识。独立离线准备可按ADR-0024先行，真实检索仍保留VPJ-50激活门。

2026-09-10 [品牌工程调整](research/VISEPANDA-BRAND-ENGINEERING-ADJUSTMENTS-2026-09-10.md)增加服务任务延续、基础跨Trip偏好和响应内容/语气验收。任务完成、事实/动作正确、表达质量与成本分别计量；正常可答场景的全拒答和重复追问计失败，软分不能抵消越权/未确认写入/重复扣次。复用既有Harness及配对入口，真实模式和holdout边界保持。

2026-09-09 增补：[Harness 可靠性规划](harness/README.md) 将两条核心任务、12场景、失败定位与恢复拆为 VPJ-66…71，纳入同一任务manifest。先离线准备、再真实集成；模型候选与Trip整合可并行。原65项产品验收/依赖保持，Harness结果作为相关能力发布的必需补充，未通过时不能把该能力声明为完成可靠性验收。
...
## 22. 基线代码复用与归档（2026-09-05快照）

下表记录2026-09-05基线：816个跟踪文件、56个server TS、20个API route、24条迁移、8个知识模块，当时main尚无已提交iOS。这些是历史库存；现已有原生代码、后续迁移与部分真实Staging验证。当前状态以[实施证据](harness/IMPLEMENTATION-STATUS.md)、对应Issue/PR和代码为准。
...
| 必须保留 | 原24条及后续已应用迁移、拒绝/权限/恢复测试、原始VP品牌资产、原工作树未跟踪iOS/Marketing | 不因新规划而删除 |

该基线存在最小`fact_records`RLS探针表，不能称零知识表；但没有生产知识schema/已审核语料。Trip已有Day/Item/snapshot代码，不能采纳PR186“只剩title”作重写理由。SSE当前仅metadata，不代表真实流式回答；Ops原文读取和媒体外发需要新合同。
...
| 新收费方向 | ADR-0027接受首轮订阅设计；价格/周期/容量待验证，年订阅/交易深度仍按证据扩展；旧Pass兼容 |
...
2026-09-05初始安排是在基线合并后，先并行完成原生壳/双语、现有Staging验证、数据政策、商品配置与客户发现；再接原生登录和真实Ask，然后共同规划/材料/知识与在途能力逐条贯通。每条依赖由Issue队列展示，不让所有任务先等待同一个大平台工程。
...
以下为2026-09-05初始容量估算，不能用作当前剩余工期。初始队列含65个子任务：60个覆盖完整首发与经营交付，5个明确后续触发任务；198条任务依赖。逐项占位估算合计247专注工作日，其中首发231日，包含不少外部/经营准备投入，**不是247日开发承诺或按3个Agent除以3的上线日期**。实际观察窗口、审核等待、知识许可和真机/境内网络验证另列。
...
波次有并行和前置准备，不是必须完成整个波次才能开始下一波。第一次看见真实iOS成果在W0/W1，不等待全部任务结束才验证核心体验。实际首两周吞吐、返工和外部条件会决定是否调整范围或团队容量；每次调整在Program和主报告同步。

2026-09-05页面冒烟曾发现：从英文入口跳到`/homepage`会回到中文，属于已有locale连续性缺陷，由VPJ-01/41迁移修复；老页面小字/对比和较多工程语言也进入新视觉/文案验收，不能把当前无水平溢出当成最终UIUX达标。

## 29. 2026-09-05规划交付与迁移历史

初始Program为[#187](https://github.com/JTCAO515/VP-V4/issues/187)，65个子任务实际编号为#188–#252，按依赖顺序创建（VPJ逻辑编号不等同GitHub顺序）。20个旧开放Issue已全部closed/not_planned并链接替代责任；198条native依赖与父子关系、正文在2026-09-05T04:06:51Z完成回读校验。两个原PR185/186保留。
...
当前任务结构由[issue-plan.json](program/2026-09-05/issue-plan.json)生成，初始65/198统计不覆盖后续Harness/HF增量。最新状态、前置、运行证据与未决项从[Program入口](program/2026-09-05/README.md)、当前Issue/PR和[实施状态](harness/IMPLEMENTATION-STATUS.md)读取；结构表不是已完成清单。

#188验收原生基础屏幕、最低系统和可访问性。已登录真实Trip选区Ask/确认/重载由#198实现，#233复验整链UX，#242完成真实设备贯通；不能让它们反向阻塞所依赖的基础任务。#188既有裁切/VoiceOver等未验项仍保留，职责澄清不等于完成验收。
