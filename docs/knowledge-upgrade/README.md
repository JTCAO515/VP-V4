...
**2026-09-15 VPJ-75切片3已落地，分支未推送**（[docs/contracts/wiki-generation-draft-content.md](../contracts/wiki-generation-draft-content.md)、[artifacts/VPJ-75](../../artifacts/VPJ-75/)）：切片2发现`wiki_page_revisions`没有字段存模型真实生成的`{summary, gaps}`正文（只有change_note≤400字），本切片新增`draft_content jsonb not null`列，回填旧行、复用现有`closed_object()`helper做shape check（严格`{summary,gaps}`两键、summary1-600字、gaps最多5条），并让`ops_wiki_generation_v1`的complete/succeeded分支必须携带并写入`draftContent`。本次会话环境无Docker/Supabase CLI，改用Homebrew装node/pnpm/gh后：`pnpm lint`/`typecheck`/`docs:check`全过，`pnpm test:contract`359/359过（含更新的wiki-schema测试）；数据库侧未用项目自己的`supabase db reset --local`（无Docker），改为独立本地Postgres16+手搭的auth stub（真实`auth.uid()`/`auth.jwt()`读取`request.jwt.claim(s)` GUC，非真实GoTrue/PostgREST），跑通全部54条历史migration+本片、6组CHECK反例、回填正确性、以及一次真实认证actor的claim→complete round trip（含"complete不带draftContent应拒绝"和"带draftContent应成功并原样落库"两个关键断言）。分支`feat/vpj-75-359-wiki-draft-content`仅存在于本地clone，本沙盒无该仓库GitHub推送凭证，尚未push/开PR。#359仍OPEN，Ops审查UI/statement抽取/矛盾处理/Docling集成均未做；见`artifacts/VPJ-75/unrun.md`。
