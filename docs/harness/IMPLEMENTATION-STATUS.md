...
2026-09-12：[开发接入规则](../agents/development-integration-policy.md)取消第三方/法务/产品审批阻塞。下列带日期的状态保留当时技术事实，历史资格/许可等待不再限制当前开发；按实际接口和运行缺口接续。
...
| VPJ-67 #264 | OPEN。VPJ-16 #206已于2026-09-16关闭（PR416），本票在issue正文列出的直接Blocked by（VPJ-66 #263、VPJ-16 #206）均已解除；`status:blocked`标签已按此证据移除。但VPJ-07 #195仍依赖VPJ-04/05/06/59的真实身份、provider、预算和数据政策全链尚未完成Staging集成（见下节"阻塞边界"），真实只读问答的完整验收仍未达成。本轮新增H04（歧义澄清，见[VPJ-67-READONLY-SEEDS.md](VPJ-67-READONLY-SEEDS.md)）离线接通并实际跑通，12例中H01/H02/H04共3例已运行；H03/H07/H08/H12等其余VPJ-67场景仍NOT_RUN并如实记录未接通原因。不能把这些离线fixture进展当只读真实问答完成，#264保持OPEN。 |
...
Staging只读迁移列表已重新确认26，27/28仍未应用。公开Preview登录bundle指向该新加坡项目；管理接口403，服务端和Production配置仍待核实。DB28严格协议与legacy客户端存在切换间隙，应先盘点所有调用端并取得明确升级/配置/维护范围；回退flag不能恢复legacy兼容。远程原生仍缺受控配置契约，不能将loopback本机证明转写为远端就绪。2026-09-11已收到硬锁暂缓与C2/团队权限方向，见下节；不取消原依赖。

## 2026-09-11 产品决定增量

JT已明确长期保存、删除聊天/账号后仍保留正文、团队成员跨用户全部CRUD长期权限、模型字段无额外限制、真实测试每日预算无用户指定上限且不再等待账单；`***`无独立语义。详见[版本化决定](../policy/vpj-03-text-decisions.md#2026-09-11-决定增量vpj03-jt-20260911-v1)。上文2026-09-10“待决定”及预算是历史状态，不再作为重复提问依据。

首版用户硬锁明确暂缓；不是锁保护已经通过，#265已确认晚餐保护案例仍未验。真机人工验收按用户报告通过；具体未指明专项及自动失败保留。团队身份/成员退出、权限审计、删除与告知合同、provider地域/资格及真实持久Ask仍须实现和验证；本次决定不打开RLS、不执行远端迁移、不关闭#190/#192或Harness运行票。
