...
同目标必要澄清、输出修复、断线恢复沿用业务归属；明显独立目标可记录新 ServiceTask。新 ID 不等于已获准的新收费单位。完成后自主改稿、扩大范围的计费分界须等待相应策略批准，并在可能产生新消费前明确告知用户。
...
等待用户 TTL 和费用待核期限须分开。释放预留后的继续执行需要重新检查合法容量，不能复活已到期预留、重置 task 已花成本或默认从下一段 Pass 扣取。上述跨期行为在策略批准前保持不可用。
