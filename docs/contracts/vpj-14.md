...
The page is a single small form plus the latest 50 candidates and expandable audits. All labels are synchronized in `lib/i18n.ts` for zh/en/es/ru/ar; Arabic updates document language and direction. Current data is fetched without cache and cleared on failed reads, auth change and tab re-entry. Mutation failures clear the workspace and require refresh; they never optimistically mark a candidate reviewed. Browser-held text is not a revocation mechanism; every server action rechecks database authority.
