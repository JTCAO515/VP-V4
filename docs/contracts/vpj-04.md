...
Authenticated access to existing public RLS tables has an additional restrictive mobile-session predicate. Writes take the same account lock as login/logout, then recheck active session state. Existing authenticated-callable public `SECURITY DEFINER` RPCs also check at entry, including no-write idempotent replay branches. The migration freezes all 15 ordinary mobile-RPC signatures and original source hashes, and rejects unknown inventories/bodies. `append_chat_turn_event` is explicitly service-role-only; `capture_initial_trip_version` is a trigger implementation with no Data API role execution grant. Both are revoked before the inventory runs, and the append-only ACL migration repairs earlier deployed databases. Removing the inserted guard must reproduce the original body hash. Original business/owner/TripProposal checks remain in place. Future tables/RPCs must explicitly retain these guards before exposing a native consumer.
