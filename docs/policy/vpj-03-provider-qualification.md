...
## 已观察到的账号与公开资料（保留运行线证据）

[2026-09-11控制台记录](../../artifacts/VPJ-03/qwen-account-20260911/metadata.json)确认千问按量付费分区已启用VP-v4凭据，Base URL与当前配置相符；没有展开或重置完整密钥。
[平台条款与价格记录](../../artifacts/VPJ-03/qwen-account-20260911/public-evidence.md)记录公开接收实体与最高档标准价格。
[工单回执](../../artifacts/VPJ-03/qwen-account-20260911/support-receipt.json)仅保留当时发送事实，工单不是开发依赖，不再催办。
这些历史artifact中的审批结论由当前开发规则替代；实际模型调用、告知/同意与结果持久化另记真实验证。
