...
版本 `VPJ03-PROVIDERS-20260912-v2`；核验日期2026-09-11；责任人JT。
状态：公开证据准备完成，真实C2资格未通过；Related to #190。
以 [JT决定](vpj-03-text-decisions.md)、[VPJ-07执行契约](../contracts/vpj-07.md)为边界。
公开条款、控制台与实际配置是可接受的供应商依据；不强制额外取得供应商书面答复或账号合同回执。
公开资料不替代实际用户同意和运行权限；每个接收方单独记录。
...
| --- | --- | --- | --- | --- | --- |
| Qwen：千问平台，个人账号；并不等于个人Token Plan | 千问API文档Q1关联DashScope；百炼协议Q2 §1.1签约方为**通义云启（杭州）信息技术有限公司**。账号接受的是否为此协议待核 | `https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions`；Q1给base URL，完整Chat路径须与运行器一致 | Q3将此DashScope域名映射至北京接入/存储；接入地域与推理部署范围是不同字段。实际运行endpoint和该账号部署范围未归档，不能推导全球/境内推理均已获准 | Q2生效2026-09-02；§5.2个人Token/Coding Plan允许输入输出服务改进，终止不追溯撤销已有授权。Q4区分按量API/团队版不训练；Q5说明调用数据仍会存储。具体输入/输出/备份TTL与删除回执能力unknown | 按量SKU已核；公开证据可进入告知/配置审阅，不再等待工单。运行未激活；不能套用所有阿里服务或旧接收方 |
| GLM：bigmodel.cn，个人账号 | G1定义经营者、服务提供者、处理者均为**北京智谱华章科技股份有限公司**；G2为用户协议 | `https://open.bigmodel.cn/api/paas/v4/chat/completions`（G3）；Coding专用endpoint是另一服务，不替换使用 | G1 §五对中国境内运营产生的个人信息声明境内存储、公有云；没有具体城市/推理节点/境外来源路径证据 | G1生效2025-05-20；允许匿名化数据用于机器学习/模型训练，不能写绝不训练。最短必要期限，无统一正文TTL；删除后备份可暂存并限制处理。工单或service@zhipuai.cn为申请途径，非VisePanda删除成功回执 | 候选未启用；公开资料中的限制和未确认项进入告知/配置审阅，不要求供应商额外书面确认 |
| DeepSeek：platform.deepseek.com，账号类型unknown | D1开放平台协议：**杭州深度求索人工智能基础技术研究有限公司**；个人/企业均可，账号对应唯一主体 | `https://api.deepseek.com/chat/completions`（D3）；支持的`/v1`形式不等于已调用形式 | D2 §五声明境内运营产生的个人信息境内存储；不能据此填写具体推理城市或美国/俄罗斯输入跨境依据 | D1生效2026-04-29；D2生效2026-02-10，涵盖API且有专项政策优先规则。智能对话部分列训练和界面退出；是否覆盖本API账号unknown。正文TTL未给固定值；网络日志六个月不是正文TTL | 候选未启用；依据公开API资料记录已知/未确认安排，不要求额外账号合同回执；Chat开关不证明API已退出训练 |
...
`https://dashscope.aliyuncs.com/compatible-mode/v1`；账号页显示已实名。控制台完整凭据未被展开、重置或修改。
默认业务空间页面显示所有模型已授权，但该旧凭据与此空间的绑定关系尚未证实，不能据此改用空间域名。

这补齐了工作台产品分区与启用状态。账号接受的合同版本、与经营主体的使用关系、正文留存/删除安排及
实际处理范围仍未从页面取得。本机 Keychain 中既有凭据与控制台可见前后各15字符匹配；
只在受保护进程内读取本地值，没有输出或持久化密钥，不等于完整字节或API账号有效性验证。Qwen C2 状态继续为 `not_qualified`，
没有新增真实文本外发、模型调用、付费套餐或激活 policy。

## 当前平台条款与价格补充

2026-09-11通过已登录千问模型页面的页脚定位到平台自身条款；不是账号已接受版本的回执，
...
## 当前验收要求（2026-09-12替代旧账号补证门）

按JT明确修订，本要求适用于所有模型供应商，包括Qwen、GLM、DeepSeek及未来候选。
供应商工单、书面答复、账号接受合同的回执、精确正文/备份TTL均非必交件。
依据公开产品条款、已登录控制台和实际配置记录所选provider、SKU、endpoint、公开接收实体、
文档版本或核验日期、留存/训练/删除与地区说明。未公开或未核实的细节在审阅与告知中明确标注，
不能写成“零留存”“不训练”“已删除”或确定地区。已有明确禁用条件仍须遵守。

工单000D0RX4LS仅为可选补充，不再等待或催办。Qwen公开材料与控制台信息已具备，
下一动作是按这些证据完成版本化中英告知和实际运行配置；未部署或未同意不能记为已启用。

真实文本仍需所选单一provider、准确endpoint、接收实体、运行来源/处理/存储范围、
双语notice/hash及有效期/复核日期与用户同意绑定。运行路由、接收方、权限标识等强制字段
不能以占位符代替；供应商未公开的TTL等说明性事实可如实标为未知，不再单独否决文档资格。
条款/endpoint/接收方实质变更按原契约更新版本与所需同意；不新增fallback或取消RLS/撤回约束。
