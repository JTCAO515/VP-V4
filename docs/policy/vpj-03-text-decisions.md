...
| 范围 | 用户决定 | 实施边界 |
| --- | --- | --- |
| 历史正文 | 长期保留；删除聊天、删除账号后也保留正文 | 新的留存方向尚未实施；需版本化区分隐藏/停用与实际删除，不得向用户虚称已清除正文。不复活已有撤回/删除数据；现行删除执行合同变更与用户告知另行落实。 |
...
| 处理数据与字段 | 用户不设额外字段限制，实施仍按分类/用途/资格 | 用户输入、模型回答、必要Trip上下文分别列字段/分类/目的/动作；禁止整包默认外发 |
| 地区与接收方 | decision_required | DB、函数、模型实际地区；主provider及最多一条fallback逐一接收实体/endpoint/合同依据 |
| 设备内、服务器及供应商留存 | 用户选择长期保留 | 聊天/账号删除后仍保留是新产品方向，尚未实施；供应商TTL不由本决定改变，详见v1增量 |
| 用户告知/同意 | decision_required | 中英告知版本、授权范围、确认动作与可核验receipt；营销/研究许可不替代产品AI处理许可 |
| 撤回与删除 | 已收到删除后仍保留方向；执行合同与告知待落实 | 撤回生效后禁止的新处理、删除各存储位置的期限与receipt、供应商限制、备份保留/恢复屏蔽及例外 |
...
告知通过不等于后端执行通过；必须与真实撤回、删除、权限与拒绝路径一致。PR301已验证本机手动Trip消费者；远端拒绝AI后的完整可用路径仍须实际验证。
