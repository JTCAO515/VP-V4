...
拟议数据范围：团队制作的中英合成旅行任务、合成工具输入与输出；不含真实对话、个人信息、真实Trip或用户材料。JT于2026-09-10明确允许三家进行多轮旅行文本测试，具体合成任务由agent选择。授权仅限本轮合成推理，不扩展到真实用户数据、充值或未确认的训练用途。测试结果只允许记录合同约定的脱敏元数据与评测结论，不因此获得发布原文或供应商返回内容的许可。
...
| 允许字段、用途、动作、输出/派生物处理范围 | 已批准本轮合成旅行文本推理 | 已批准本轮合成旅行文本推理 | 已批准本轮合成旅行文本推理 | 不含真实对话、个人信息、真实Trip/材料；只保存脱敏元数据与评测结论；训练另定 |
...
服务入口、两家账号类型和逐家预算已收到；不重复索取这些值。实际服务类型/endpoint、条款适用与处理地区仍须核验。2026-09-10安全检查仅确认当前进程存在DEEPSEEK_API_KEY，未找到Qwen/GLM环境凭据；这不证明账号权限或密钥有效。不得把任一家结果当三家对照通过。

DeepSeek公开[隐私政策](https://cdn.deepseek.com/policies/zh-CN/deepseek-privacy-policy.html)版本2026-02-10涵盖API及其他产品，同时列有模型训练用途和退出方式；本账号API是否受专项条款/退出设置覆盖尚未确认，不能从通用Chat设置推导API不训练。已向JT提出仅本轮虚构文本是否接受供应商留存/模型改进用途的独立问题；未收到答复前不填为已批准。精确正文TTL仍unknown，不能写零留存。
