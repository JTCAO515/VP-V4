...
| TODAY-01 | View one prepared next action and current conditions (`TODAY.next`, `conditions`) | Today Engine / #103, #104 | `partial` — deterministic one-Fact NextAction and timestamped observation projection exist; no owner reader or live condition is rendered | Current Trip, eligible Fact/Observation and expiry | `unavailable` or stale/recheck; cached values never appear live | contract: eligible NextAction and current/stale/unavailable observation; planned: owner reader | `reuse contract/test`; retire prepared weather/AQ values |
| TODAY-02 | Open delay recovery | Recovery / #105 | `partial` — timestamped delay evidence creates only an in-memory pending proposal; accept/reject/conflict/recheck are contract-covered | ExternalObservation + current Trip version | No evidence returns unavailable; Trip stays unchanged and no cancellation/purchase occurs | contract: delay proposal, stale recheck, accept/reject/version conflict; planned: owner reader and canonical writer integration | `reuse contract/test`; only a Proposal may be ported |
| TODAY-03 | Open place-closure recovery | Recovery / #105 | `partial` — timestamped closure evidence creates only an in-memory pending proposal; accept/reject/conflict/recheck are contract-covered | Eligible closure observation + exact place ID | No current closure evidence returns recheck/unavailable; Trip stays unchanged | contract: closure proposal, stale recheck, accept/reject/version conflict; planned: exact place/owner reader and canonical writer integration | `reuse contract/test`; retire static closure as fact |
| TODAY-04 | Open queue recovery | Recovery / #106 | `partial` — current timestamped queue evidence can create only an in-memory pending Proposal | User observation + reviewed alternatives | Invalid/stale evidence is unavailable/recheck; Trip stays unchanged | contract: queue Proposal plus accept/reject/conflict/recheck; planned: owner reader and alternatives | `reuse contract/test`; never claim live queue data |
| TODAY-05 | Open unwell recovery and official channel | Safety Recovery / #106 | `partial` — ordinary current evidence creates only an in-memory pending Proposal; high-risk evidence returns a current recorded official-channel reference | User input, reviewed safety guidance and official channel | High-risk missing/stale/non-recorded/non-official channel is unavailable; no diagnosis, rescue, or contact occurs | contract: ordinary Proposal and high-risk opaque official-channel reference; planned: owner reader and reviewed guidance | `reuse contract/test`; no medical diagnosis or rescue claim |
| TODAY-06 | Run and expand nine Trip checks; send an alternative to Diff (`SIM_CHECKS`) | Constraint Engine / #89, Today / #103 | `partial` — fixed nine-check registry emits evidence availability or unknown only; no Diff action exists | Deterministic constraints plus eligible evidence | Unknown inputs remain unknown; no free-form LLM verdict | contract: fixed registry; planned: final-state and corrections | `reuse contract/test`; prices/times remain fixtures |
...
| USER-03 | Export or delete privacy data | Privacy / #102 | `partial` — owner-scoped all-data request and immutable requested receipt exist; no export or deletion executor exists | Owner-scoped export/delete workflow and receipts | Failed/partial deletion never displays success; backup exception is visible | contract: closed request, receipt and no-inline-delete; planned: delivery, execution and retention observation | `reuse contract/test`; retire local confirmation toggle |
