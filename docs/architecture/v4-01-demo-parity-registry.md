...
| TODAY-01 | View one prepared next action and current conditions (`TODAY.next`, `conditions`) | Today Engine / #103, #104 | `partial` — deterministic one-Fact NextAction contract exists; no owner Trip/Fact reader or live condition is rendered | Current Trip, eligible Fact/Observation and expiry | `unavailable` or stale/recheck; cached values never appear live | contract: eligible/fail-closed NextAction; planned: owner reader and observations | `reuse contract/test`; retire prepared weather/AQ values |
