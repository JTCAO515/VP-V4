...
| CANVAS-02 | View schematic Map | Canvas Place / #96 | `fixture-only` | TripPlaceReference/UserPlaceRef + licensed map or schematic | No provider keeps a labelled schematic; no live-route claim | planned: exact-ID and provider-unavailable | `port behavior`; retire Demo coordinates as geography |
...
| MEMORY-01 | Browse 12 prepared memory items, source and impact | Memory Contract / #98, Copilot / #99 | `fixture-only` | Explicit/confirmed MemoryReceipt under owner RLS | Missing consent/source returns candidate, paused or unavailable | planned: retrieval, source and abstention | `reuse contract/test`; retire fictional traveler profile |
| MEMORY-02 | Confirm, reject, pause, forget or edit memory | Copilot / #99 | `fixture-only` | Durable owner action and audit | Failure stays visible; UI never pretends deletion/confirmation | planned: action/reload consistency | `port behavior`; rewrite local state buttons |
| MEMORY-03 | View before/after Trip impact | Memory Consumers / #100 | `fixture-only` | Concrete MemoryReceipt -> Proposal/Trip origin | Paused/rejected memory cannot affect new Turns | planned: impact trace and correction | `reuse contract/test`; no inferred item becomes a hard constraint |
...
| USER-01 | View/edit account and save | Auth / #84, Profile / #101 | `partial` — route code exists; real session acceptance incomplete | Verified user JWT + owner RLS | Unauthorized/expired session returns five-language auth failure | planned: session/profile reload | `rewrite`; retire fictional Michael Turner account |
| USER-02 | View Travel Profile, Memory and Preferences tabs | Profile / #101, Memory / #98–#100 | `fixture-only` | Explicit settings separated from Memory | Missing persistence is visible; no inferred setting masquerades as explicit | planned: owner/reload/locale-unit | `port behavior`; retire fixture defaults |
