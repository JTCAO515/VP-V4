...
Related to #193. This slice is offline preparation, not a provider benchmark or live gateway.
...
| GLM | `glm-5.3-flash` | `thinking.type: disabled`; text `json_object` mode; `sensitive` finish reason is blocked; cache details use `prompt_tokens_details`. | [Chat completion](https://docs.bigmodel.cn/api-reference/模型-api/对话补全) |
...
Provider-native JSON Schema strict mode, built-in search/retrieval, multimodal input, reasoning,
...
No provider calls, real model output, real usage/cost, account lookup or configuration writes ran.
