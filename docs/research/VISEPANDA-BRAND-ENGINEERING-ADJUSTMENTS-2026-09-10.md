...
提交前更新：主线推进到 `6b475cff27bfea17db50fa244ce5d1bd5337503f`，新增 [PR #284](https://github.com/JTCAO515/VP-V4/pull/284) 的持久模型预算实现及 [运行契约](https://github.com/JTCAO515/VP-V4/blob/6b475cf/docs/contracts/vpj-59.md)。本地 PostgreSQL/Supabase 验证有记录，新增预算迁移尚未在 Staging 应用，实际 worker/provider/Ask 消费者仍待接入。下文与附件对d4eeb084中内存预算的判断是历史快照，实施须复用新主线的durable预算，不能重复建设。正文与执行计划已据此更新，原技术安全语义保持。
...
当前 #194 已有可复用的持久预算代码，应先完成获准环境与真实消费者接入，不等待 IAP 商品，也不再建设第二套预算。#227 则负责用户容量与商业权益的整合。Q37 不取消反滥用、预算和原始成本记录，也不授权新增支付行为。
...
| #194 成本预算 | 复用PR #284的持久预算，接入实际worker/provider并核对未知费用 | 保留已有不依赖 IAP 的独立责任；新迁移与真实环境门不跳过 |
