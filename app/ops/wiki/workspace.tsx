...
export function OpsWikiWorkspace() {
  ...
  return <main className={styles.workspace} lang={locale}>
    ...
    {result && <section className={styles.list}>
      ...
      {result.revisions.map((revision, i) => { const withdrawnCited = citedWithdrawnSources(revision.sources); return <article className={styles.panel} key={revision.id}>
        <h3>{i === 0 ? c.current : c.previous} · v{revision.version} · {c[revision.validationStatus]}</h3>
        {withdrawnCited.length > 0 && <p role="alert" className={styles.conflict}>{c.citesWithdrawnSource(withdrawnCited.length)}</p>}
