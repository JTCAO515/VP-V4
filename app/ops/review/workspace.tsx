...
import { wikiReviewTarget, wikiReviewContext, type WikiReviewContext } from "@/lib/server/knowledge/wiki/review-context";
import type { WikiRead } from "@/lib/server/knowledge/wiki/read-model";
...
export function OpsReviewWorkspace() {
  ...
  const [signInTarget, setSignInTarget] = useState("/ops/review");
  const [wikiDraft, setWikiDraft] = useState<WikiReviewContext | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (wikiTarget) {
        const wikiResponse = await fetch(`/api/ops/wiki?pageKey=${encodeURIComponent(wikiTarget.pageKey)}`, { cache: "no-store", signal: AbortSignal.timeout(10000) });
        const wikiResult = await wikiResponse.json();
        if (current !== generation.current) return;
        const context = wikiResponse.ok ? wikiReviewContext(wikiResult.data as WikiRead, wikiTarget) : null;
        if (!context) { setWikiError(true); return; }
        setWikiDraft(context); setWithStatement(true); setWithSource(false);
      }
  ...
  useEffect(() => {
    ...
    const subscription = client?.auth.onAuthStateChange((event) => {
      if (event === "SIGNED_OUT" || event === "SIGNED_IN" || event === "USER_UPDATED") {
        ++generation.current; inFlight.current = null; setBusy(false); setWorkspace(null); setWikiDraft(null); setPending(null); form.current?.reset(); setMessage(null);
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!workspace) return;
    if (withStatement) {
      const statement = statementFields(values);
  ...
  return <main className={styles.workspace}>
    ...
    <section className={styles.list} aria-label={c.heading}>{workspace.candidates.length === 0 && <p>{c.empty}</p>}
      {workspace.candidates.map((candidate) => <article className={styles.panel} key={candidate.id}>
        ...
        {candidate.wikiOrigin && <p className={styles.meta}>{locale === "zh" ? "运营整理自 Wiki 草稿" : "Operator statement from Wiki draft"}: {candidate.wikiOrigin.pageKey} · v{candidate.wikiOrigin.version} · {candidate.wikiOrigin.revisionId}</p>}
