...
import { isMemoryConsentInput, isSameOriginMutation } from "@/lib/server/identity/request-guards";
...
export async function POST(request: NextRequest) {
  ...
  if (!isMemoryConsentInput(input)) return NextResponse.json(failureResponse("INVALID_INPUT"), { status: 400 });
  ...
  const result = await adapter.setMemoryConsent(input);
  if ("error" in result) {
