...
import { suggestPlaces } from "@/lib/server/maps/provider-suggest-adapter";
...
const text = (value: string | null, max: number): value is string => typeof value === "string" && value.trim().length > 0 && value.length <= max;
...
export async function GET(request: NextRequest) {
  ...
  if (!provider(selectedProvider) || !text(query, 200) || (city !== null && !text(city, 100))) return reply({ error: { code: "INVALID_INPUT" } }, 400);
  ...
  try {
    return await scope.run(async () => {
      ...
      const outcome = await suggestPlaces({ provider: selectedProvider, query, city: city ?? undefined, env: process.env, fetcher: scope.fetch });
      scope.check();
