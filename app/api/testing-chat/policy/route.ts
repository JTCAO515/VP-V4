...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await lifetime.run(() => rpc.call("read_grounded_policy", { p_policy_id: config.policyId }));
    if (error || !data || (data as Record<string, unknown>).kind !== "policy") return rpc.applyCookies(response({ error: "TESTING_CHAT_UNAVAILABLE" }, 503));
