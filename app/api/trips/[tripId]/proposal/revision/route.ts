...
import { isProposalRevisionInput, isSameOriginMutation, isTripProposalRevisionInput, isUuid } from "@/lib/server/identity/request-guards";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tripId: string }> }) {
  ...
  if (!isProposalRevisionInput(input) && !isTripProposalRevisionInput(input)) return NextResponse.json(failureResponse("INVALID_INPUT"), { status: 400 });
  ...
  const result = isTripProposalRevisionInput(input) ? await adapter.revisePendingProposalPatch(tripId, input) : await adapter.revisePendingProposal(tripId, input);
  if ("error" in result) {
