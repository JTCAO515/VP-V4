...
import { isProposalRejectInput, isSameOriginMutation, isUuid } from "@/lib/server/identity/request-guards";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ tripId: string }> }) {
  ...
  if (!isProposalRejectInput(input)) return NextResponse.json(failureResponse("INVALID_INPUT"), { status: 400 });
  ...
  const result = await adapter.rejectPendingProposal(tripId, input);
  if ("error" in result) {
