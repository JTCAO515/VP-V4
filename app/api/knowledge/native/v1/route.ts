...
export async function GET(request:Request){
 ...
 const input=knowledgeReadScope(new URL(request.url));if(!input)return failure('INVALID_INPUT',400);
 ...
 try{return await scope.run(async()=>{
  ...
  const {data,error}=await credentials.client.rpc('knowledge_read_v1',{p_input:input}).abortSignal(scope.signal);scope.check();
