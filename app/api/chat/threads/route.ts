...
import { isChatThreadInput, isSameOriginMutation } from "@/lib/server/identity/request-guards";
...
export async function GET(request: NextRequest) {
  ...
  const result = await adapter.listChatThreads();
  if ("error" in result) {
...
export async function POST(request: NextRequest) {
  ...
  if (!isChatThreadInput(input)) return NextResponse.json(failureResponse("INVALID_INPUT"), { status: 400 });
  const adapter = createUserDataAdapter(request);
  if (!adapter) return NextResponse.json(failureResponse("PROVIDER_UNAVAILABLE"), { status: 503 });
  const result = await adapter.createChatThread(input.tripId);
  if ("error" in result) {
    const failure = failureResponse(result.error);
    return adapter.applyCookies(NextResponse.json(failure, { status: failure.status }));
  }
