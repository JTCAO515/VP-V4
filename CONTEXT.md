...
The tracker plan is published as V4-01 through V4-31 while preserving R0-R5. It adds ContextPlan, Tool Policy Gateway, ConstraintEngine and a rejectable RoutePattern RAG spike. LangChain/LangGraph/GraphRAG/multi-agent are not default implementation work. V4-13 now supplies the owner-scoped Memory Profile lifecycle, consent, source/lifecycle receipts and safe retrieval projection; it has no Copilot/UI/provider consumer yet. V4-14, V4-16, V4-18 and V4-22 through V4-30 are now merged as truthful five-locale/RTL unavailable surfaces where durable inputs or reviewed high-risk content do not exist; none activates a provider, offline cache, TTS, state mutation, artifact upload or fixture fallback. The V4-31 audit covers all 40 registry actions and has a blocked, non-release verdict until V4-15/V4-17/V4-19–V4-21 and runtime evidence exist. The Continuous AFK scheduler must recompute native dependencies and labels after every PR or merge; label synchronization is skipped while `gh` lacks an authenticated user.
