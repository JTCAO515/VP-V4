...
- Current frontier: #120 AI-51a blocks #84 after production evidence proved the Magic Link PKCE verifier was created only in ephemeral server memory, leaving the callback browser `UNAUTHENTICATED`. #120 persists the verifier cookie in the initiating browser; #84 and #15 remain open until deployment and real owner/other-user evidence pass. #87 V4-01 is closed.
...
The tracker plan is published as V4-01 through V4-31 while preserving R0-R5. It adds ContextPlan, Tool Policy Gateway, ConstraintEngine and a rejectable RoutePattern RAG spike. LangChain/LangGraph/GraphRAG/multi-agent are not default implementation work. #120 is the only ready frontier; it blocks #84, which continues to block #15 and the R1 gate.
