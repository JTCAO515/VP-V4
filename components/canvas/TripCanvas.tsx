...
export function TripCanvas({ tripId }: { tripId: string }) {
  ...
  return <Shell><main className={styles.main}>
    ...
    <section className={styles.history}><article className={styles.panel}><p className={styles.eyebrow}>Version history</p><h2>Confirmed changes</h2>{data.versions.length === 0 ? <p>No confirmed changes have been recorded for this Trip yet.</p> : <ol className={styles.timeline}>{data.versions.map((version) => <li className={styles.event} key={version.id}><i className={styles.dot} aria-hidden="true"/><div><strong>{version.eventType === "initial" ? "Initial version" : `Version ${version.resultingVersion} confirmed`}</strong><span>{version.eventType === "initial" ? "The immutable baseline for this Trip." : "Applied through an immutable Proposal."}</span><span className={styles.meta}>Recorded {new Date(version.createdAt).toLocaleString()}</span>{version.proposalId && <details><summary>Inspect origin Proposal</summary><span className={styles.meta}>Proposal ID: {version.proposalId}</span></details>}{version.memoryReceipts.length > 0 ? <details><summary>Memory provenance</summary><ul>{version.memoryReceipts.map((receipt) => <li key={`${receipt.memoryId}-${receipt.sourceReceiptId}`}>{receipt.constraintKind === "hard_constraint" ? "Hard constraint" : "Preference"} · <Link href="/visepanda/copilot">Memory source {receipt.memoryId}</Link> · receipt {receipt.sourceReceiptId}</li>)}</ul></details> : null}{version.resultingVersion < data.trip.headVersion && version.title !== null && <button className={styles.button} disabled={action !== null || pendingRollback !== null} onClick={() => void prepareRollback(version.resultingVersion)}>{action === "preparing" ? "Preparing…" : `Restore this version as v${data.trip.headVersion + 1}`}</button>}</div></li>)}</ol>}</article><div><TripPlaceView tripId={tripId}/><TripActionsView tripId={tripId}/><aside className={styles.panel}><p className={styles.eyebrow}>Integrity</p><h2>One current head</h2><p>The timeline is read from the same owner-scoped Trip source as the current version. It has no fixture fallback and no direct write action.</p></aside></div></section>
  </main></Shell>;
}
