...
type LoadState = "loading" | "ready" | "unavailable" | "unauthenticated" | "no_current_trip";
type Copy = Readonly<{ eyebrow: string; title: string; loading: string; unavailable: string; noCurrentTrip: string; signIn: string; signInBody: string; home: string; language: string; asOf: string; today: string; upcoming: string; noItems: string; complete: string; incomplete: string; openTrip: string }>;

const copy: Record<Locale, Copy> = {
  zh: { eyebrow: "今日", title: "已确认行程的下一步", loading: "正在读取已确认行程。", unavailable: "当前无法读取行程。", noCurrentTrip: "目前没有已确认行程。", signIn: "登录", signInBody: "请登录后查看你的已确认行程。", home: "VisePanda 首页", language: "界面语言", asOf: "时间依据", today: "今天", upcoming: "即将开始", noItems: "已确认行程中没有条目。", complete: "该行程没有后续日期条目。", incomplete: "行程日期或时区不完整，无法可靠选择下一步。", openTrip: "打开行程" },
  en: { eyebrow: "Today", title: "The next step from your confirmed Trip", loading: "Reading your confirmed Trip.", unavailable: "Your Trip cannot be read right now.", noCurrentTrip: "There is no confirmed Trip yet.", signIn: "Sign in", signInBody: "Sign in to view your confirmed Trip.", home: "VisePanda home", language: "Interface language", asOf: "Clock", today: "Today", upcoming: "Upcoming", noItems: "There are no items in this confirmed Trip.", complete: "This Trip has no remaining dated items.", incomplete: "A Trip date or timezone is incomplete, so the next step cannot be selected reliably.", openTrip: "Open Trip" },
  es: { eyebrow: "Hoy", title: "El siguiente paso de tu viaje confirmado", loading: "Leyendo tu viaje confirmado.", unavailable: "Tu viaje no puede leerse ahora.", noCurrentTrip: "Aún no hay un viaje confirmado.", signIn: "Iniciar sesión", signInBody: "Inicia sesión para ver tu viaje confirmado.", home: "Inicio de VisePanda", language: "Idioma de la interfaz", asOf: "Reloj", today: "Hoy", upcoming: "Próximamente", noItems: "No hay elementos en este viaje confirmado.", complete: "Este viaje no tiene más elementos con fecha.", incomplete: "Falta una fecha o zona horaria del viaje; no se puede elegir el siguiente paso de forma fiable.", openTrip: "Abrir viaje" },
  ru: { eyebrow: "Сегодня", title: "Следующий шаг из подтверждённой поездки", loading: "Читаем подтверждённую поездку.", unavailable: "Сейчас поездку нельзя прочитать.", noCurrentTrip: "Подтверждённой поездки пока нет.", signIn: "Войти", signInBody: "Войдите, чтобы увидеть подтверждённую поездку.", home: "Главная VisePanda", language: "Язык интерфейса", asOf: "Время", today: "Сегодня", upcoming: "Скоро", noItems: "В этой подтверждённой поездке нет пунктов.", complete: "В этой поездке не осталось датированных пунктов.", incomplete: "Дата или часовой пояс поездки неполны, поэтому следующий шаг нельзя выбрать надёжно.", openTrip: "Открыть поездку" },
  ar: { eyebrow: "اليوم", title: "الخطوة التالية من رحلتك المؤكدة", loading: "جارٍ قراءة الرحلة المؤكدة.", unavailable: "لا يمكن قراءة رحلتك الآن.", noCurrentTrip: "لا توجد رحلة مؤكدة بعد.", signIn: "تسجيل الدخول", signInBody: "سجّل الدخول لعرض رحلتك المؤكدة.", home: "الصفحة الرئيسية لـ VisePanda", language: "لغة الواجهة", asOf: "الوقت", today: "اليوم", upcoming: "قريباً", noItems: "لا توجد عناصر في هذه الرحلة المؤكدة.", complete: "لا تحتوي هذه الرحلة على عناصر مؤرخة متبقية.", incomplete: "تاريخ الرحلة أو منطقتها الزمنية غير مكتمل، لذلك لا يمكن اختيار الخطوة التالية بشكل موثوق.", openTrip: "فتح الرحلة" },
};
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!currentTripId) return setState("no_current_trip");
