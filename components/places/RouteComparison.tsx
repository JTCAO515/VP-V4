...
export function RouteComparison({ selected, chinese }: { selected: RoutePlace | null; chinese: boolean }) {
  ...
  return <section className={styles.detail} aria-label={text("Compare routes", "比较路线")}>
    ...
    <label>{text("Departure", "出发时间")} <select aria-label={text("Departure", "出发时间")} value={future ? "future" : "now"} onChange={event => { invalidate(); setFuture(event.target.value === "future"); }}><option value="now">{text("Now", "现在")}</option><option value="future">{text("Future departure", "未来出发")}</option></select></label>
