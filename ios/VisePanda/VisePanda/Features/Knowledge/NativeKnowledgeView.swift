    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: VPSpacing.section) {
                BrandHeader()
                Text(text("Travel notes", "旅途参考")).font(.largeTitle.bold())
                Text(text("Reviewed information for your selected city and situation. Read each note's conditions and check its source before taking action. This is not complete city coverage.", "按所选城市和场景提供已审核信息。请阅读每条内容的适用条件，行动前核对来源；这里不代表完整城市覆盖。"))
                ...
                Picker(text("Situation", "场景"), selection: selectionBinding($scene)) {
        ...
        .task(id: loadKey) {
            ...
            repeat {
                ...
                await store.load(scope: key.scope, selection: key.selection) {
                    try await session.knowledgeRequest(selection: key.selection)
                }
