...
function native(command,args,log,env){return new Promise((resolve,reject)=>{const file=createWriteStream(log);let output='';const child=spawn(command,args,{cwd:repo,env,stdio:['ignore','pipe','pipe']});for(const stream of [child.stdout,child.stderr])stream.on('data',part=>{const text=part+'';file.write(text);output=(output+text).slice(-8192);});child.once('error',()=>reject(new Error('Native simulator process launch failed')));child.once('exit',code=>{file.end();if(code===0)return resolve();const diagnostic=output.split(/\r?\n/).filter(line=>/error:|failed|XCTAssert|Test Case|TEST FAILED/i.test(line)).slice(-4).join(' ').replace(/https?:\/\/[^\s]+/g,'<redacted-url>').replace(/[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+/g,'<redacted-email>').replace(/[A-Za-z0-9._-]{12,}/g,'<redacted>');reject(new Error(`Native simulator verification failed: ${diagnostic||'no safe diagnostic'}`));});});}
