...
try{
  ...
  exit=await run(process.execPath,['--test','tests/integration/turn/native-text-http.test.mjs'],true,{...process.env,VP_NATIVE_TEXT_INTEGRATION:'true',VISEPANDA_TRIP_PROTOCOL_V2:'true',VP_IDENTITY_SUPABASE_WORKDIR:target,VP_IDENTITY_SUPABASE_API_URL:`http://127.0.0.1:${base+21}`,VP_NATIVE_API_PORT:String(base+31)});
}finally{
