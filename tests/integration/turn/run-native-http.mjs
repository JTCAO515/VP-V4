...
if(args.length>1 || (args.length===1 && !['--service','--grounded','--grounded-native'].includes(args[0])))throw Error('Unknown native HTTP test option');
const base=59620;
...
try{
  ...
  exit=await run(process.execPath,['--experimental-strip-types',...(args[0]==='--grounded-native'?['tests/integration/turn/run-native-grounded-events.mjs']:['--test',args[0]==='--grounded'?'tests/integration/turn/native-grounded-http.test.mjs':'tests/integration/turn/native-text-http.test.mjs'])],true,{...process.env,VP_NATIVE_TEXT_INTEGRATION:'true',VP_NATIVE_GROUNDED_INTEGRATION:args[0]==='--grounded'?'true':'false',VP_NATIVE_TEXT_SERVICE_INTEGRATION:args[0]==='--service'?'true':'false',VISEPANDA_TRIP_PROTOCOL_V2:'true',VP_IDENTITY_SUPABASE_WORKDIR:target,VP_IDENTITY_SUPABASE_API_URL:`http://127.0.0.1:${base+21}`,VP_NATIVE_API_PORT:String(base+31)});
}finally{
