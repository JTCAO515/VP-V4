...
test("V4-12 exposes only owner action references and rejects user writes", async (t) => {
  const env = identityLocalEnv(); if (!env?.API_URL || !env.ANON_KEY || !env.SERVICE_ROLE_KEY) return t.skip("explicit disposable identity Supabase target is not configured");
