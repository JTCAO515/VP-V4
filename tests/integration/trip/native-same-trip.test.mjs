...
test('local native/Web share one immutable Trip with confirmed intent, CAS and ordinary-owner isolation', {skip:process.env.VP_LOCAL_SAME_TRIP!=='true',timeout:180000},async t=>{
 ...
 native=await n('/'+tripId);web=await w('/'+tripId);assert.deepEqual(native.data.content,web.data.content);assert.equal(native.data.trip.headVersion,1);assert.equal(native.data.confirmationState,'confirmed');assert.equal(native.data.hardLocks,'not_enabled');assert.equal(native.data.externalOrderStatus,'not_connected');assert.deepEqual({hardLocks:web.data.hardLocks,externalOrderStatus:web.data.externalOrderStatus},{hardLocks:'not_enabled',externalOrderStatus:'not_connected'});
