...
test("freezes exactly the 24 registered failure codes with unique metric labels", () => {
  assert.equal(FAILURE_CODES.length, 24);
  assert.equal(Object.keys(FAILURE_TAXONOMY).length, 24);
  assert.equal(new Set(Object.values(FAILURE_TAXONOMY).map((item) => item.metricLabel)).size, 24);
