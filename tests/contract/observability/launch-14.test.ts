...
test("LAUNCH-14 correlates API, Turn, worker, and provider metadata without accepting caller trace IDs", () => {
  const chain = createContentFreeTraceChain({ mintTraceId: () => traceId });
  const events = [
  ...
  assert.deepEqual(events.map((event) => event.traceId), [traceId, traceId, traceId, traceId]);
  ...
  assert.throws(() => chain.record({ stage: "api", outcome: "started", latencyMs: 1, retryCount: 0, inputTokens: 0, outputTokens: 0, costMicros: 0, traceId }));
});
