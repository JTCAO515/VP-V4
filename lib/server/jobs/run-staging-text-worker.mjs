...
import {TEXT_TURN_PROMPT_REF} from '../model-gateway/prompt/text-turn.ts';
...
try{
 ...
 const worker=createStagingTextJob(JSON.parse(contents),{
 ...
 await append({schemaVersion:'vpj07-worker-run/2',configurationDigest,prompt:TEXT_TURN_PROMPT_REF,phase:'started',observedAt:new Date().toISOString()});
 ...
 await append({schemaVersion:'vpj07-worker-run/2',configurationDigest,prompt:TEXT_TURN_PROMPT_REF,phase:'returned',result,observedAt:new Date().toISOString()});
