...
try{
 ...
 const worker=createStagingTextJob(config,{
  ...
  qwenEndpoint:readQwenEndpoint(process.env),
  recordKnowledgeValidation:async(receipt)=>{await append({schemaVersion:'vpj07-knowledge-validation-journal/1',configurationDigest,receipt});},
  recordUsage:async(receipt,signal)=>{if(signal.aborted)throw Error('unavailable');await append({schemaVersion:'vpj07-usage-journal/1',configurationDigest,receipt});if(signal.aborted)throw Error('unavailable');},
 ...
 const prompt=config.inputMode==='knowledge_intent_v1'?KNOWLEDGE_INTENT_PROMPT_REF:config.inputMode==='task_history_v1'?TEXT_TASK_PROMPT_REF:TEXT_TURN_PROMPT_REF;
 const generation=config.thinkingBudgetTokens===undefined?{}:{generation:{mode:'qwen-bounded-thinking-v1',maxCompletionTokens:config.budget.maxOutputTokens,thinkingBudgetTokens:config.thinkingBudgetTokens}};
 ...
 await append({schemaVersion:config.thinkingBudgetTokens===undefined?'vpj07-worker-run/2':'vpj07-worker-run/3',configurationDigest,prompt,...generation,phase:'started',observedAt:new Date().toISOString()});
 ...
 await append({schemaVersion:config.thinkingBudgetTokens===undefined?'vpj07-worker-run/2':'vpj07-worker-run/3',configurationDigest,prompt,...generation,phase:'returned',result,observedAt:new Date().toISOString()});
