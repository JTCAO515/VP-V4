...
export function wikiReviewTarget(search: string) {
  ...
  return { pageKey, revisionId, version };
}
export function wikiReviewContext(read: WikiRead, target: NonNullable<ReturnType<typeof wikiReviewTarget>>): WikiReviewContext | null {
  ...
  return sources.length ? {pageKey:read.pageKey,revisionId:r.id,version:r.version,summary:r.draftContent.summary,gaps:r.draftContent.gaps,sources} : null;
}
