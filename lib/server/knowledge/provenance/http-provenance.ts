import { isProvenanceReadInput } from "./ontology.ts";
...
/** Read-only counterpart of handleOpsRequest for ops_knowledge_provenance_read_v1. No body to stream — factId travels as one query param. */
export async function handleProvenanceRequest(request: Request, options: {
  ...
  if (keys.length !== 1 || keys[0] !== "factId" || !isProvenanceReadInput({ factId })) return response("INVALID_INPUT", 400);
  ...
  try {
    ...
    lifetime.check();
    const { data, error } = await lifetime.run(() => rpc.call("ops_knowledge_provenance_read_v1", { p_input: { factId } }));
    if (error) {
