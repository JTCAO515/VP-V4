...
const raw=readFileSync(root+'/artifacts/VPJ-02/staging-33-preparation/migrations.json');
const manifest=JSON.parse(raw);assert.equal(manifest.targetCount,33);assert.equal(manifest.migrations.length,33);
...
writeFileSync(output+'/supabase/config.toml','project_id = "vpj02-staging-33-frozen"\n',{flag:'wx',mode:0o600});
...
console.log(JSON.stringify({result:'FROZEN_PACKAGE_CREATED',count:33,manifestSha256:sha(raw),laterMigrationsExcluded:excluded,
