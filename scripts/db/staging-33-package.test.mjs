...
const manifest=JSON.parse(readFileSync(root+'/artifacts/VPJ-02/staging-33-preparation/migrations.json','utf8'));
function fixture(t){
  const temp=mkdtempSync(join(tmpdir(),'vpj02-package-test-'));
  t.after(()=>rmSync(temp,{recursive:true,force:true}));
  const source=join(temp,'source');mkdirSync(source);
  const files=['scripts/db/staging-33-package.mjs','scripts/db/staging-33-rehearsal.mjs','artifacts/VPJ-02/staging-33-preparation/migrations.json',...manifest.migrations.map(m=>m.file),...manifest.administrativePreparation.map(m=>m.file)];
  ...
  const run=()=>spawnSync(process.execPath,[source+'/scripts/db/staging-33-package.mjs','--out',out],{encoding:'utf8'});
  return {source,out,run};
}
test('frozen package emits exact33 and administrative hashes while excluding future migrations/config',t=>{
  const f=fixture(t);
  ...
  assert.equal(result.count,33);assert.equal(result.remoteActions,0);assert.equal(result.linked,false);
  assert.deepEqual(result.laterMigrationsExcluded,['20990101000000_future_not_authorized.sql']);
  const verify=spawnSync(process.execPath,[f.source+'/scripts/db/staging-33-rehearsal.mjs','--verify'],{encoding:'utf8'});
  assert.equal(verify.status,0,verify.stderr);
  assert.equal(readdirSync(f.out+'/supabase/migrations').length,33);
  ...
  assert.equal(readFileSync(f.out+'/supabase/config.toml','utf8'),'project_id = "vpj02-staging-33-frozen"\n');
