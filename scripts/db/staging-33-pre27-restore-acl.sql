-- NOT an automatic failure handler. Explicit operator rollback only while still at26.
-- Restores previously observed broad execution privileges; never run after27/mobile activation.
begin;
...
begin
  ...
  for e in select * from (values
    ('public.append_chat_turn_event(uuid,text,text,text)','87b75d363d22ee004c3d8a34da97ca60','record','{postgres=X/postgres,service_role=X/postgres}'),
    ('public.capture_initial_trip_version()','cd64e5a9c72ddc91461765abe18258f1','trigger','{postgres=X/postgres,service_role=X/postgres}'),
    ('public.rls_auto_enable()','99be20677b456ea8d3be47bdd44fb369','event_trigger','{postgres=X/postgres,service_role=X/postgres}')
  ...
  if (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and has_function_privilege('authenticated',p.oid,'EXECUTE'))<>15 then raise exception 'VPJ02_UNREVIEWED_DEFINER_INVENTORY'; end if;
end;
$preflight$;
grant execute on function public.append_chat_turn_event(uuid,text,text,text) to anon,authenticated;
grant execute on function public.capture_initial_trip_version() to anon,authenticated;
grant execute on function public.rls_auto_enable() to anon,authenticated;
grant execute on function public.rls_auto_enable() to public;
do $verify$ begin
  if (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and has_function_privilege('authenticated',p.oid,'EXECUTE'))<>18 then raise exception 'VPJ02_RESTORE_FAILED'; end if;
end; $verify$;
commit;
