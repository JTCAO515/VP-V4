...
const root='/Users/jtcao/Library/Caches/visepanda/place-v7-boundaries-20260914',repo=root+'/frozen';
...
try{
 ...
 assert.equal(scenario.maxNewModelAttempts,6);assert.equal(deployment.readyState,'READY');assert.notEqual(deployment.target,'production');assert.equal(deployment.sha,scenario.apiSourceCommit);
 ...
 within();phase='login-'+sample.id;const u=JSON.parse(fs.readFileSync('/Users/jtcao/Library/Caches/visepanda/s2-live-brrfsvtp/accounts.json','utf8')).users.find(x=>x.locale===sample.locale);const token=await login(u);
 within();phase='admit';const request={threadId:randomUUID(),turnId:randomUUID(),idempotencyKey:randomUUID(),policyId:scenario.policyId,locale:sample.locale,city:'shanghai',text:sample.text,serviceTask:{id:randomUUID(),scopeVersion:1,relationship:'new_goal',parentTurnId:null}};
 const task={sample,request};state.tasks.push(task);save();task.admission=await call(base+'/turns',{token,body:request});save();assert.equal(task.admission.status,201);
 phase='worker';task.worker=await worker(sample.locale,state.tasks.length);save();assert.equal(task.worker.exitCode,0);assert.equal(task.worker.result,'finished');assert.equal(task.worker.prompt.digest,scenario.promptDigest);
 const records=fs.readFileSync(root+'/worker-'+state.tasks.length+'.jsonl','utf8').trim().split('\n').map(JSON.parse);
 ...
 assert.equal(task.diagnosis.reason,'valid');assert.equal(task.turn.result.intent,sample.intent);assert.equal(task.turn.result.requestScope,sample.scope);if(sample.scope==='unknown')assert.deepEqual(task.turn.result.unansweredNeeds,[]);else {const needs=task.turn.result.unansweredNeeds;assert.equal(needs.length,1);assert.ok(needs[0].includes(sample.needs[0]));assert.ok(!/Cedar Moon Gallery|address|杉月展馆|地址/i.test(needs[0]));}task.matches=true;save();await logoutAll();console.log(JSON.stringify({case:sample.id,status:'PASS',calls:state.workerLaunches}));
 }
 state.status='PASS';state.originalAcceptance='FAIL_UNCHANGED';save();
