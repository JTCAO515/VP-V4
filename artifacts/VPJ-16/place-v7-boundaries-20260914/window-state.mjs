...
try {
 if(mode==='prepare'){
  assert.ok(!fs.existsSync(file('activation-intent.json')));const s=snapshot();assert.equal(s.state.migrations,48);assert.equal(s.state.users,6);assert.equal(s.state.trips,3);assert.equal(s.state.attempts,351);assert.equal(s.state.unresolved,0);assert.equal(s.state.ops,false);assert.equal(s.state.reader,false);assert.equal(s.state.activeMembers,0);assert.equal(s.state.members.length,2);assert.ok(s.state.members.every(x=>!x.active));const allowedPublicationHashes=[];save('before.json',{at:new Date().toISOString(),...s,allowedPublicationHashes},true);console.log({mode,status:'PASS',state:s.state});
