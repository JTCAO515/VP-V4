...
import {query,env,dockerArgs} from '../specific-gaps-20260914/transport.mjs';
...
const intact=(before,after)=>{for(const [table,hashes] of Object.entries(before.original)){const current=new Map();for(const h of after.original[table])current.set(h,(current.get(h)||0)+1);for(const h of hashes){assert.ok(current.get(h)>0,'Original row changed: '+table);current.set(h,current.get(h)-1);}}};
...
const ownershipSql=xid=>`(select count(*) from knowledge_review_private.settings where xmin::text='${BigInt(xid)%4294967296n}')=1 and (select count(*) from knowledge_review_private.publication_settings where xmin::text='${BigInt(xid)%4294967296n}')=1 and (select count(*) from knowledge_review_private.members where actor_id in ('${owners.join("','")}') and xmin::text='${BigInt(xid)%4294967296n}')=2 and pg_current_xact_id()::text::numeric>=${xid} and pg_current_xact_id()::text::numeric-${xid}<2147483648`;
const mutate=(before,activation,xid)=>{
 const revisionCheck=before.state.members.map(m=>`(actor_id='${m.id}' and revision=${m.revision+(activation?0:1)} and active=${activation?'false':'true'})`).join(' or ');
 return `do $$ begin if (select count(*) from knowledge_review_private.members where ${revisionCheck})<>2 or (select count(*) from knowledge_review_private.members where active)<>${activation?0:2} or (select enabled from knowledge_review_private.settings) is distinct from ${activation?'false':'true'} or (select enabled from knowledge_review_private.publication_settings) is distinct from ${activation?'false':'true'} ${activation?'':`or not (${ownershipSql(xid)})`} then raise exception 'WINDOW_OWNERSHIP_CHANGED';end if;end $$;update knowledge_review_private.members set active=${activation},revision=revision+1 where actor_id in ('${owners.join("','")}');update knowledge_review_private.settings set enabled=${activation};update knowledge_review_private.publication_settings set enabled=${activation};select ${summary};commit;`;
};
// Journal the DB-assigned transaction before any mutation. A lost commit response remains
// recoverable by matching all four row xmins, even when activated.json was never written.
const activate=before=>new Promise((resolve,reject)=>{
 const runId=randomUUID(),binding={project:'dzqdzetcctkhbrhlxxgn',owners,rows:['knowledge_review_private.settings:singleton','knowledge_review_private.publication_settings:singleton',...owners.map(x=>'knowledge_review_private.members:'+x)],members:before.state.members,beforeHash:createHash('sha256').update(fs.readFileSync(file('before.json'))).digest('hex')};durableSave('activation-intent.json',{at:new Date().toISOString(),runId,...binding});
...
try {
 if(mode==='prepare'){
  assert.ok(!fs.existsSync(file('activation-intent.json')));const s=snapshot();assert.equal(s.state.migrations,46);assert.equal(s.state.users,6);assert.equal(s.state.trips,3);assert.equal(s.state.attempts,315);assert.equal(s.state.unresolved,0);assert.equal(s.state.ops,false);assert.equal(s.state.reader,false);assert.equal(s.state.activeMembers,0);assert.equal(s.state.members.length,2);assert.ok(s.state.members.every(x=>!x.active));assert.equal(s.original['knowledge_review_private.statements'].length,12);save('before.json',{at:new Date().toISOString(),...s},true);console.log({mode,status:'PASS',state:s.state});
