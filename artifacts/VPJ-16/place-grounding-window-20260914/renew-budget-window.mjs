...
assert.equal(before.scopes.length,2);for(const c of configs){const s=before.scopes.find(x=>x.id===c.budget.scopeId);assert.equal(s.owner_id,c.ownerId);assert.equal(s.limit_micros,70000000);assert.equal(s.enabled,true);assert.equal(s.frozen,false);assert.ok(Date.parse(s.expires_at)<Date.now());}
