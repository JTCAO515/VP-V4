...
const root='/Users/jtcao/Library/Caches/visepanda/evidence-lifecycle-r3-20260914',repo=root+'/frozen';
...
async function observe(stage){
 ...
 for(const u of users){const token=await login(u),reply=await call(base+'/turns',{token});assert.equal(reply.status,200);
  for(const task of state.tasks.filter(t=>t.locale===u.locale)){const turn=reply.body.turns.find(t=>t.turnId===task.request.turnId);assert.ok(turn);entry.native.push({group:task.group,locale:u.locale,turn});state.inProgressObservation=entry;save();const knowledge=turn.result.knowledge,[coverage,count]=expect(stage,task);assert.equal(knowledge.answer.outcome,coverage);assert.equal(knowledge.statements.length,count);assert.equal(turn.result.originalOutcome,task.group==='before'?'answered':'partial');
   ...
   if(stage===3)assert.deepEqual([...card.reasons].sort(),scenario.stages[stage].oldCardReasons);
...
try{
 assert.ok(!exists('lifecycle.json'),'An existing run must be inspected, never rerolled');for(const item of scenario.statements)assert.ok(isOpsInput({action:'submit_statement',operationId:randomUUID(),candidateId:item.candidateId,title:item.title,statement:item.statement}),'Frozen statement rejected by actual Ops validator');assert.equal(scenario.priorRun.modelAttempts+scenario.maxNewModelAttempts,scenario.maxSliceModelAttempts);assert.ok(!exists('activation-intent.json'));assert.equal(state.scenarioHash,fs.readFileSync(root+'/scenario.sha256','utf8').trim());assert.equal(deployment.readyState,'READY');assert.notEqual(deployment.target,'production');assert.equal(deployment.sha,scenario.sourceCommit);assert.equal(read('budget-preflight.json').status,'PASS');assert.ok(Date.now()-Date.parse(read('budget-preflight.json').at)<10*60000);
