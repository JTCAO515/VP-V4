...
try{
 ...
 const listing=await call();state.listingStatus=listing.status;save();assert.equal(listing.status,200);assert.ok(Array.isArray(listing.body.data?.candidates),'Expected Ops candidate listing');
 for(const item of state.operations){
  ...
  for(let i=0;i<2;i++){
   ...
   let result;try{result=await call(body);}catch(e){item.attempts[i].transportError={name:e.name,message:e.message==='fetch failed'?'fetch failed':'Private transport details suppressed'};save();if(stopped||i===1)throw e;continue;}
...
}catch(e){state.status='FAIL';state.error={name:e.name,message:e.name==='AssertionError'?e.message:e.message==='fetch failed'?'fetch failed':'Private transport details suppressed'};save();process.exitCode=1;}
finally{
