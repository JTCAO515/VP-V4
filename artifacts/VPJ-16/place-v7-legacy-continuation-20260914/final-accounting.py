...
if v['status']=='PASS':
 assert v['workerLaunches']==s['maxNewModelAttempts']==86
 assert len(v['tasks'])==86 and all(t.get('matches') and t.get('foreignOwnerHidden') and t.get('replayBound') for t in v['tasks'])
 assert len(diagnostics)==86 and all(d['reason']=='valid' for d in diagnostics)
...
summary={'schemaVersion':'place-continuation-accounting/1','runStatus':v['status'],'workerCalls':len(receipts),'matchedCases':sum(bool(t.get('matches')) for t in v['tasks']),'unrunCases':86-len(v['tasks']),'tariffCnyMicros':sum(fees.values()),'tariffByOwner':dict(fees),'supplierInvoice':'unknown','originalRowHashesPreserved':True,'unresolved':0,'readerOpsMembersClosed':True,'firewallRestoredVersion':f1['version'],'originalAcceptanceFailuresRemain':True,'recoveredReadMatched':bool(v.get('recoveredRead',{}).get('matches')),'startedAt':v['startedAt'],'finishedAt':v['finishedAt']}
