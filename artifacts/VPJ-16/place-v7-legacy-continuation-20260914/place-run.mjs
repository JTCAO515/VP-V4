...
const root='/Users/jtcao/Library/Caches/visepanda/place-v7-legacy-continuation-20260914',repo=root+'/frozen';
...
const call=async(path,{token,actor,body}={})=>{
 for(let attempt=0;attempt<(body?1:2);attempt++){
  try{const r=await fetch(origin+path,{method:body?'POST':'GET',redirect:'error',signal:AbortSignal.timeout(30000),headers:{...(token?{authorization:'Bearer '+token}:{}),...(actor?{Cookie:actor.cookie()}:{}),...(body?{'Content-Type':'application/json',...(actor?{Origin:origin}:{})}: {})},...(body?{body:JSON.stringify(body)}:{})});return{status:r.status,body:await r.json()};}
  catch(e){if(body||attempt!==0||!['TypeError','TimeoutError'].includes(e.name))throw e;state.transportRetries??=[];state.transportRetries.push({at:new Date().toISOString(),method:'GET',path,errorType:e.name});save();}
 }
};
...
try{
 ...
 const prior=JSON.parse(fs.readFileSync(scenario.continuationOf));assert.ok(prior.finishedAt);assert.equal(prior.workerLaunches,46);assert.equal(prior.tasks.length,46);
 assert.equal(createHash('sha256').update(fs.readFileSync(scenario.continuationOf)).digest('hex'),scenario.originalRunHash);
 const originalPath='/Users/jtcao/Library/Caches/visepanda/place-v7-isolated-20260914/scenario.json';assert.equal(createHash('sha256').update(fs.readFileSync(originalPath)).digest('hex'),scenario.originalScenarioHash);assert.deepEqual(scenario.legacy,JSON.parse(fs.readFileSync(originalPath)).legacy.slice(20));
 assert.equal(read('before.json').state.migrations,48);assert.equal(read('before.json').state.attempts,405);assert.equal(scenario.maxNewModelAttempts,86);assert.equal(scenario.legacy.length,86);
 ...
 const token=await login(users.find(x=>x.locale===interrupted.locale));const result=await call(base+'/turns',{token});assert.equal(result.status,200);const turn=result.body.turns.find(t=>t.turnId===scenario.recoverTurnId);assert.ok(turn);matches(interrupted.expected,turn);
 ...
 await produce('legacy',scenario.legacy);assert.equal(state.workerLaunches,86);state.status='PASS';save();
