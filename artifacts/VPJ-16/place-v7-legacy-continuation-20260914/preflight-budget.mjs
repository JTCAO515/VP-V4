...
const root='/Users/jtcao/Library/Caches/visepanda/place-v7-legacy-continuation-20260914';
...
writeFileSync(root+'/budget-preflight.json',JSON.stringify({at:new Date().toISOString(),status:'PASS',scope:'next admission plus headroom only; sequential full plan may stop at unchanged database cap',fullPlanWorstCaseFunded:false,plannedCallsPerOwner:43,rows:result},null,2)+'\n',{mode:0o600});
console.log(JSON.stringify({status:'PASS',scope:'two existing budgets; next admission headroom, not86 worst-case reservations',fullPlanWorstCaseFunded:false,unresolved:0,configurationChanged:false}));
