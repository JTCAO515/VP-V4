...
if v['status']=='PASS':
 assert v['workerLaunches']==s['maxNewModelAttempts']==85
 assert len(v['tasks'])==85 and all(t.get('matches') and t.get('foreignOwnerHidden') and t.get('replayBound') for t in v['tasks'])
 assert len(diagnostics)==85 and all(d['reason']=='valid' for d in diagnostics)
 assert {d['turnId'] for d in diagnostics}=={t['request']['turnId'] for t in v['tasks']}=={x['turnId'] for x in receipts}
 assert v['recoveredRead']['matches'] and v['recoveredRead']['foreignOwnerHidden'] and v['recoveredRead']['modelCalls']==0
 assert v['recoveredRead']['turnId']==s['recoverTurnId']
 assert all(x.get('status') in ['PASS',200] for x in v['cleanup'])
fees=Counter()
for x in receipts:fees[x['attempt']['ownerId']]+=x['actualMicros']
summary={'schemaVersion':'place-continuation-accounting/1','runStatus':v['status'],'workerCalls':len(receipts),'matchedCases':sum(bool(t.get('matches')) for t in v['tasks']),'unrunCases':85-len(v['tasks']),'tariffCnyMicros':sum(fees.values()),'tariffByOwner':dict(fees),'supplierInvoice':'unknown','originalRowHashesPreserved':True,'unresolved':0,'readerOpsMembersClosed':True,'firewallRestoredVersion':f1['version'],'originalAcceptanceFailuresRemain':True,'recoveredReadMatched':bool(v.get('recoveredRead',{}).get('matches')),'startedAt':v['startedAt'],'finishedAt':v['finishedAt']}
