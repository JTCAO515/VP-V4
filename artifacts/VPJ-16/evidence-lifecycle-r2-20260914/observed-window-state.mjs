...
try {
 if(mode==='prepare'){
  assert.ok(!fs.existsSync(file('activation-intent.json')));const s=snapshot();assert.equal(s.state.migrations,46);assert.equal(s.state.users,6);assert.equal(s.state.trips,3);assert.equal(s.state.attempts,317);assert.equal(s.state.unresolved,0);assert.equal(s.state.ops,false);assert.equal(s.state.reader,false);assert.equal(s.state.activeMembers,0);assert.equal(s.state.members.length,2);assert.ok(s.state.members.every(x=>!x.active));assert.equal(s.original['knowledge_review_private.statements'].length,13);save('before.json',{at:new Date().toISOString(),...s},true);console.log({mode,status:'PASS',state:s.state});
