...
const root='/Users/jtcao/Library/Caches/visepanda/place-v7-isolated-20260914',repo=root+'/frozen';
...
async function retained(stage){
 ...
 for(const actor of actors){const r=await call('/api/chat/grounded',{actor});assert.equal(r.status,200);
  for(const task of state.tasks.filter(t=>t.locale===actor.locale&&['before','current'].includes(t.group))){
   ...
   if(stage!=='revoked'){assert.ok(turn.facts.some(f=>f.placeDetails?.some(t=>t.includes('18 Willow Dawn Test Road')))||hoursOnly);}
...
try{
 ...
 assert.equal(new Date(Date.now()+8*3600000).toISOString().slice(0,10),scenario.localDate);assert.equal(read('before.json').state.migrations,48);
 ...
 assert.equal(read('fixture-isolation.json').status,'PASS');assert.ok(Date.now()-Date.parse(read('fixture-isolation.json').at)<10*60000);assert.equal(read('budget-preflight.json').status,'PASS');assert.ok(Date.now()-Date.parse(read('budget-preflight.json').at)<10*60000);assert.equal(scenario.maxNewModelAttempts,132);assert.equal(scenario.legacy.length,106);
 ...
 phase='publish-today';await publish('today-hours');await retained('current');await produce('current',scenario.current);await retained('current');
