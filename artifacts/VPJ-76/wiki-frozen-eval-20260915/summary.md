...
覆盖率 90.9%，过拒答率 9.1%（相对同批结构化/直接读取baseline，共3条分歧，均为刻意构造的 retrieval_miss/budget_exhausted 场景）。p50 0.112ms / p95 0.719ms（fixture transport，非真实网络/模型延迟）。累计 usage token：960（fixture声明值，非真实计费）。
