...
- #551 concurrently edits `NativeTripStore`, `NativeTripView` and `NativeTripStateTests`. This branch adds no store change. The Today test was moved away from #551's insertion point; `git merge-tree --write-tree HEAD refs/remotes/origin/pr-551` returned a tree with no conflict at #551 head `5e12275c`. If either head changes, repeat the overlap check; the eventual combined native build/tests remain separate evidence.
...
- Preparation remains a request-local, expiring carrier SIM document check. Today names that narrow scope, opens it with the current Trip version and does not claim a persisted readiness state or general Trip readiness.

## 2026-09-26 integration with merged #551

- Merged `origin/main` at `15e9be8e` (#551 Trip deletion) into this branch without conflict. Today remains a read-only consumer. If deletion clears the selected Trip detail, the Today screen no longer renders its earlier in-memory version.
- PASS: `xcodebuild test -project ios/VisePanda/VisePanda.xcodeproj -scheme VisePanda -destination 'platform=iOS Simulator,id=B0AD77FD-33C3-4616-92CE-2E76ACD93148' -derivedDataPath /tmp/vpj25-t1-combined -only-testing:VisePandaTests/NativeTripStateTests CODE_SIGNING_ALLOWED=YES CODE_SIGN_IDENTITY=-` — 15 tests, 0 failures, iOS 17.5 simulator.
- PASS: simulator generic build with `-derivedDataPath /tmp/vpj25-t1-combined CODE_SIGNING_ALLOWED=NO`; `pnpm docs:check`; `git diff --check origin/main...HEAD`.
- The address, remote identity, entitlement and device limits above remain unchanged. Repository CI on the combined head is pending; local combined tests do not establish authenticated or physical-device acceptance.
