...
## Main coordinator's later authenticated ASC readback (2026-09-25 about 08:55 Asia/Shanghai)

The main coordinator regained an authenticated App Store Connect session and read the same **VisePanda** app record `6813077089`: target upload `0.1.0 (20260924.163820)` **Complete**; target build **Internal / Testing**, linked to **VisePanda Internal**, with about 90 days before expiry. Its stable build detail showed **Groups (1)**, that group's single tester **Accepted**, and two group builds. Metadata showed binary **Validated**, bundle `space.go2china.VisePanda`, minimum iOS 17.0, arm64 iPhone support, and distribution entitlements `NLD68762GG.space.go2china.VisePanda`, `get-task-allow=false`, `beta-reports-active=true`. This later authenticated readback supersedes the earlier browser-login limitation for those named fields. It does not establish that Apple's downloadable package was available to JTs17 at 08:36; the observed installation remains **FAIL** and its cause **unknown**.

The Business page showed the **Free Apps Agreement active through 2027-07-03** and the **Paid Apps Agreement in a new / view-and-agree state**, unsigned. Nothing was signed or changed. Apple's [App Store Connect workflow](https://developer.apple.com/help/app-store-connect/get-started/app-store-connect-workflow) describes the Paid Apps Agreement for selling apps or offering In-App Purchases; its [agreement guidance](https://developer.apple.com/help/app-store-connect/manage-agreements/sign-and-update-agreements) does not establish that signing it is a prerequisite for this internal TestFlight download. Whether the unsigned paid agreement contributed to this error is **unknown**. Do not sign it solely to make this failure disappear without separate business authority and evidence.
