...
- V4-12/#97 is reviewed and ready to merge: Canvas Reservations & Actions reads owner-scoped durable Trip references only. An HTTPS URL is a recorded external link, never an official-channel claim; missing links are unavailable and `recheck_required` remains visible. A future artifact source needs its own owner-bound receipt contract. No route creates orders, payments, inventory or provider completion. `pnpm check`, contract (56), E2E (8) and security (17 plus one local-runtime skip) passed; local Supabase integration remains skipped because the runtime is unavailable.
