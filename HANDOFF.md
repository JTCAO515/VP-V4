...
- #87 V4-01 is closed. Production evidence then found #120 AI-51a: `/api/auth/magic-link` returned 202, but the clicked callback remained `401 UNAUTHENTICATED` because the PKCE verifier lived only in discarded server memory. #120 moves verifier storage to the initiating browser cookie boundary, without service credentials or RLS changes.
- #120 is the only ready frontier and blocks #84; #84 continues to block #15. Merge is not acceptance: the repaired deployment still needs a same-browser Magic Link callback plus owner/other-user Trip route evidence.
